...
const mockOutfits = [
  {
    ...
    accentLight: "#FDE8EC",
    pieces: [
      { name: "Lehenga Skirt",  detail: "Deep crimson silk with zardozi embroidery" },
    ...
    fabric: "Silk · Organza",
    occasion: "Barat",
    stylistNote: "The cinched waist lehenga will beautifully define your hourglass silhouette. The backless choli adds elegance without overpowering your frame.",
  },
  {
    id: 2,
    ...
    accentLight: "#FDF0F4",
    pieces: [
      { name: "Sharara Pants",  detail: "Flared palazzo cut, floral embroidery" },
    ...
    occasion: "Mehndi",
    stylistNote: "The wide-leg sharara creates beautiful proportion. The embroidered hem draws the eye downward, elongating your figure gracefully.",
  },
  {
    id: 3,
    ...
    accentLight: "#E8F7F0",
    pieces: [
      { name: "Anarkali Frock", detail: "Floor-length flared silhouette, heavy embroidery" },
    ...
    occasion: "Walima",
    stylistNote: "The flared Anarkali skims the waist and flares beautifully from the hips, making it universally flattering.",
  },
  {
    id: 4,
    ...
    pieces: [
      ...
      { name: "Fitted Blouse",  detail: "Deep back, gold border, elbow sleeves" },
    ...
    fabric: "Banarsi Silk",
    occasion: "Nikkah",
    stylistNote: "A saree drape creates an elongating vertical line. Choose a front-pinned style to highlight your waist beautifully.",
  },
];
...
const OutfitHero = ({ outfit }) => {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden"
      style={{ background: `linear-gradient(160deg, ${p[0]}18 0%, ${p[1]}28 40%, ${p[2]}18 80%, ${p[3]}12 100%)` }}>

      {/* large fabric‑texture SVG fills the whole space */}
      <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="xMidYMid slice"
        viewBox="0 0 400 540" xmlns="http://www.w3.org/2000/svg">
        <defs>
          {/* diagonal weave */}
          <pattern id={`w-${id}`} x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
          ...
          {/* embroidery dots */}
          <pattern id={`e-${id}`} x="0" y="0" width="48" height="48" patternUnits="userSpaceOnUse">
            ...
            <path d="M24 18 Q30 24 24 30 Q18 24 24 18 Z" fill={p[0]} opacity="0.12"/>
          </pattern>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <svg viewBox="0 0 160 420" fill="none" xmlns="http://www.w3.org/2000/svg"
          style={{ width:"auto", height:"78%", maxHeight:"380px", filter:`drop-shadow(0 20px 60px ${p[0]}55)` }}>
          <defs>
          ...
          <ellipse cx="80" cy="22" rx="17" ry="20" fill={`url(#fg-${id})`} opacity="0.6"/>

          {/* neck */}
          <path d="M72 41 C72 46 88 46 88 41 L88 52 C88 56 72 56 72 52 Z" fill={`url(#fg-${id})`} opacity="0.75"/>

          {/* choli / blouse — fitted bodice */}
          <path d="M36 56 C24 60 18 70 20 82 C22 92 30 98 36 102 C48 108 60 110 72 112 L88 112 C100 110 112 108 124 102 C130 98 138 92 140 82 C142 70 136 60 124 56 C114 52 100 50 80 50 C60 50 46 52 36 56 Z"
            fill={`url(#fg-${id})`}/>

          {/* dupatta drape over shoulder — left */}
          <path d="M36 56 C28 62 22 72 20 84 C18 96 22 116 26 132 L34 148 C30 134 24 114 26 98 C28 82 34 70 40 62 Z"
            fill={`url(#lg-${id})`} opacity="0.55"/>

          {/* waistband */}
          <path d="M48 112 C56 116 68 118 80 118 C92 118 104 116 112 112 L116 124 C108 130 96 134 80 134 C64 134 52 130 44 124 Z"
            fill={`url(#fg-${id})`} opacity="0.85"/>

          {/* lehenga skirt — full flared */}
          <path d="M44 124 C36 132 24 150 16 172 C8 196 6 224 8 252 C10 276 16 298 22 316 L34 348 C30 324 24 298 22 272 C20 248 22 220 28 198 C34 178 44 158 52 144 Z"
            fill={`url(#lg-${id})`} opacity="0.80"/>
          <path d="M116 124 C124 132 136 150 144 172 C152 196 154 224 152 252 C150 276 144 298 138 316 L126 348 C130 324 136 298 138 272 C140 248 138 220 132 198 C126 178 116 158 108 144 Z"
            fill={`url(#lg-${id})`} opacity="0.80"/>
          {/* skirt front panel */}
          <path d="M52 144 C44 160 38 184 36 210 C34 238 38 268 44 294 C50 316 58 334 64 348 L96 348 C102 334 110 316 116 294 C122 268 126 238 124 210 C122 184 116 160 108 144 C100 136 92 132 80 132 C68 132 60 136 52 144 Z"
            fill={`url(#fg-${id})`} opacity="0.90"/>

          {/* skirt embroidery hem line */}
          <path d="M22 316 C40 332 60 344 80 346 C100 344 120 332 138 316" stroke="white" strokeWidth="1.5" fill="none" opacity="0.45" strokeDasharray="4 3"/>

          {/* legs */}
          <path d="M64 348 L58 420 M96 348 L102 420" stroke={`url(#fg-${id})`} strokeWidth="14" strokeLinecap="round"/>

          {/* dupatta flowing right */}
          <path d="M112 60 C124 72 132 90 136 110 C140 132 138 158 134 180 L128 198 C132 174 134 148 130 126 C126 106 118 86 108 68 Z"
            fill={`url(#lg-${id})`} opacity="0.50"/>

          {/* highlight sheen on skirt */}
          <path d="M72 144 C68 170 66 200 68 232 C70 260 76 290 82 312" stroke="white" strokeWidth="2.5" fill="none" opacity="0.30" strokeLinecap="round"/>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-40 pointer-events-none"
        style={{ background:`linear-gradient(to top, rgba(245,237,228,1) 0%, rgba(245,237,228,0.85) 30%, transparent 100%)` }}/>
    </div>
  );
};
...
const ThumbCard = ({ outfit, active, saved, onSelect, onSave }) => {
  ...
  return (
    <div
      ...
      style={{
        width: "110px",
        boxShadow: active ? "0 12px 36px rgba(120,8,28,0.22)" : "0 3px 14px rgba(180,120,100,0.13)",
      }}>
      {/* mini preview */}
      <div className="relative" style={{ height:"120px" }}>
        <div className="w-full h-full" style={{ background:`linear-gradient(145deg, ${outfit.palette[0]}25, ${outfit.palette[1]}40, ${outfit.palette[2]}25)` }}>
          <svg className="w-full h-full opacity-25" viewBox="0 0 110 120" preserveAspectRatio="xMidYMid slice">
            <defs>
              <pattern id={`tp-${outfit.id}`} width="16" height="16" patternUnits="userSpaceOnUse" patternTransform="rotate(30)">
                <line x1="0" y1="0" x2="0" y2="16" stroke={outfit.palette[0]} strokeWidth="0.5"/>
              </pattern>
            </defs>
          ...
          {/* mini silhouette */}
          <div className="absolute inset-0 flex items-center justify-center">
            <svg viewBox="0 0 40 100" fill="none" style={{ width:"36px", height:"72px" }}>
              <ellipse cx="20" cy="8" rx="7" ry="8" fill={outfit.palette[0]} opacity="0.8"/>
              <path d="M13 16 C9 18 6 23 7 30 C8 36 12 39 15 41 C13 46 10 52 10 60 C10 72 14 82 18 88 L22 88 C26 82 30 72 30 60 C30 52 27 46 25 41 C28 39 32 36 33 30 C34 23 31 18 27 16 C24 14 20 14 20 14 C20 14 16 14 13 16 Z" fill={outfit.palette[0]} opacity="0.7"/>
              <path d="M18 88 L16 100 M22 88 L24 100" stroke={outfit.palette[0]} strokeWidth="4" strokeLinecap="round" opacity="0.7"/>
            </svg>
          </div>
        </div>
        {/* match badge */}
        <div className="absolute top-1.5 left-1.5 rounded-full px-1.5 py-0.5 text-[9px] font-bold text-white"
          style={gradBg}>{outfit.match}%</div>
        {/* save */}
        <button
      ...
      <div className="px-2 py-2 bg-white/90 backdrop-blur-sm">
        ...
        <div className="flex gap-1 mt-1.5">
          {outfit.palette.slice(0,3).map(c => (
            <div key={c} className="w-3.5 h-3.5 rounded-full border border-white/70 shadow-sm" style={{ background:c }}/>
          ))}
        </div>
      </div>
    </div>
...
export default function ViewOutfitPage() {
  const userProfile = {
    event: "Barat", style: "Traditional", gender: "Female",
    season: "Winter", height: 165, skinTone: "Medium", bodyType: "Hourglass",
  };
    ...
    <div className="h-auto w-full absolute bg-[#F5EDE4] overflow-x-hidden">
      ...
      <div className="relative z-20 flex items-center justify-between px-5 py-25">
        ...
        <button onClick={() => setShowProfile(p => !p)}
          ...
          <span className="text-[11px] font-semibold text-[#4b4949]">{userProfile.event}</span>
      ...
      ════════════════════════════════════════ */}
      {active && (
        <div className="relative z-10 hero-in" key={`hero-${active.id}`}
          ...
          <div className="w-full relative" style={{ height:"72vh", minHeight:"420px", maxHeight:"620px" }}>
            ...
            <div className="absolute top-20 left-5 flex gap-1.5 z-10">
              {active.tags.map(tag => (
                <div key={tag} className="rounded-full px-2.5 py-1 text-[10px] font-bold text-white"
                  style={gradBg}>{tag}</div>
              ))}
            </div>
            ...
            <div className="absolute top-20 right-5 flex gap-2 z-10">
              ...
              <button className="w-9 h-9 rounded-2xl flex items-center justify-center text-base bg-white/80 border-2 border-white/60 text-[#9A9A9A] backdrop-blur-sm">↗</button>
            </div>
      ...
      ════════════════════════════════════════ */}
      {active && (
        <div className="relative z-10 mx-4 mb-5 rounded-3xl overflow-hidden border border-white/75 slide-up"
          ...
          <div className="px-4 py-4 fade-in" key={activeTab + active.id}>
              ...
              <div className="space-y-0.5">
                {active.pieces.map((piece, i) => (
                  <div key={i} className="piece-row flex items-center gap-3 px-2 py-3 transition-colors">
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 text-[12px] font-black text-white"
            ...
            {/* Stylist Note */}
            {activeTab === "stylist" && (
              <div className="rounded-2xl p-4 border border-[rgba(255,111,145,0.18)]"
                ...
                <div className="flex items-center gap-2.5 mb-3">
                  ...
                  <span className="text-[11px] font-black text-[#78081C] uppercase tracking-widest">Stylist's Advice</span>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-[rgba(255,111,145,0.12)] flex flex-wrap items-center gap-2">
                  ...
                  {userProfile.skinTone && (
                    <span className="flex items-center gap-1 text-[10px] font-bold text-[#961e32] bg-[rgba(255,111,145,0.10)] rounded-full px-2 py-0.5 border border-[rgba(255,111,145,0.22)]">
                      <span className="w-2 h-2 rounded-full" style={{ background: skinToneColors[userProfile.skinTone] }}/>
            ...
            {activeTab === "colours" && (
              <div className="space-y-3">
                <p className="text-[11px] text-[#9A9A9A] mb-3 leading-relaxed">
                  Colours selected to complement your skin tone and the {active.occasion} vibe.
                </p>
                {active.palette.map((color, i) => {
                  const names = active.colors.split(" · ");
                  return (
                    <div key={color} className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl border-2 border-white shadow-sm flex-shrink-0"
                        style={{ background:color }}/>
                      <div className="flex-1">
                        <p className="text-[12px] font-semibold text-[#2a2a2a]">{names[i] ?? `Accent ${i+1}`}</p>
                      ...
                      <div className="w-16 h-1.5 rounded-full bg-[rgba(220,195,170,0.3)] overflow-hidden">
                        <div className="h-full rounded-full" style={{ width:`${[55,25,15,5][i]??10}%`, background:color }}/>
                      </div>
      ...
      {/* ════════════════════════════════════════
          STYLING TIPS
      ════════════════════════════════════════ */}
      <div className="relative z-10 mx-4 mb-5 rounded-3xl p-4 border border-white/75 slide-up"
        ...
        <div className="space-y-2">
          {[
            { icon:"💡", tip:`For ${userProfile.event}, opt for rich fabrics like silk or velvet.` },
            { icon:"👟", tip:`At ${userProfile.height}cm, embroidered platforms add a perfect finish.` },
            { icon:"💎", tip:`${userProfile.skinTone} skin tones glow in jewel tones and antique gold.` },
          ].map((t,i) => (
            <div key={i} className="flex items-start gap-2.5 px-2 py-2 rounded-xl hover:bg-[rgba(255,111,145,0.05)] transition-colors">
      ...
      {saved.length > 0 && (
        <div className="relative z-10 mx-4 mb-5 fade-up">
          <p className="text-[10px] font-bold tracking-[0.13em] uppercase text-[#961e32] mb-2">Saved ({saved.length})</p>
          <div className="flex gap-2 flex-wrap">
            {mockOutfits.filter(o => saved.includes(o.id)).map(o => (
              <div key={o.id}
                ...
                <div className="w-2.5 h-2.5 rounded-full" style={{ background:o.palette[0] }}/>
                <span className="text-[11px] font-semibold text-[#78081C]">{o.title}</span>
                <button className="text-[#9A9A9A] text-xs hover:text-[#FF6F91]"
                  onClick={e => { e.stopPropagation(); toggleSave(o.id); }}>×</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="relative z-10 mx-4 mb-10 slide-up" style={{ animationDelay:"0.25s" }}>
        <div className="flex gap-3">
          ...
          <button className="flex-1 py-4 rounded-full text-white text-sm font-bold hover:-translate-y-0.5 transition-all"
            style={{ ...gradBg, boxShadow:"0 10px 32px rgba(255,111,145,0.38)" }}
